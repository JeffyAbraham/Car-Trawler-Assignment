...
const carReducer = (state = INITIAL_STATE, action) => {
  switch (action.type) {
    ...
    case "APPLY_MULTIFILTER":
     var filterdData=  multiFilter(state.carDetails, state.priceFilterId, state.supplyFilterId);
     
      return {
        ...state,filteredVersionCarDetails:filterdData
      };
    case "FIND_CAR_BY_ID":
      var current=findById(action.payload,state.carDetails)
      return{
        ...state,currentCar:current
      }
